...
import {
  SchemaReminderPatchReq,
  SchemaReminderPostReq,
  TypeReminderGet,
  TypeReminderIdGet,
  TypeReminderPatchRes,
  TypeReminderPostRes,
...
const router = Router();
router.use(...baseMiddleware, requireAuthentication);

// GET /reminders
router.get(
  "/",
  ah(async (req: AuthenticatedRequest, res, next) => {
    const users_reminders = await db
      .select()
      .from(reminders)
      .where(
...
router.get(
  ...
  ah(async (req: AuthenticatedRequest, res) => {
    ...
    const [reminder] = await db
      ...
      .from(reminders)
      .where(
        and(eq(reminders.id, numeric_id), eq(reminders.user_id, req.user_id!)),
      );
...
router.delete(
  ...
  ah(async (req: AuthenticatedRequest, res) => {
    ...
    const [reminder] = await db
      .delete(reminders)
      .where(
        and(eq(reminders.user_id, req.user_id!), eq(reminders.id, numeric_id)),
      )
      .returning();
